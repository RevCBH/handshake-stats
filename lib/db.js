            ...
            switch (_a.label) {
                case 0:
                    ...
                    switch (params.operation) {
                        case 'avg':
                            operation = slonik_1.sql(templateObject_8 || (templateObject_8 = __makeTemplateObject(["avg(", ")"], ["avg(", ")"])), series);
                            break;
                        case 'sum':
                            operation = slonik_1.sql(templateObject_9 || (templateObject_9 = __makeTemplateObject(["sum(", ")"], ["sum(", ")"])), series);
                            break;
                        default:
                            throw new Error("Invalid timeseries operation: " + params.operation + " ");
                    }
                    return [4 /*yield*/, connection.query(slonik_1.sql(templateObject_10 || (templateObject_10 = __makeTemplateObject(["\n            SELECT time_bucket(", ", time) AS label, ", " AS value\n            FROM blocks\n            GROUP BY label\n            ORDER BY label ASC\n                "], ["\n            SELECT time_bucket(", ", time) AS label, ", " AS value\n            FROM blocks\n            GROUP BY label\n            ORDER BY label ASC\n                "])), params.bucketSize, operation))];
                case 1:
                    result = _a.sent();
